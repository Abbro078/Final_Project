                ...
                if(loadBalancingManager.isRedirectRequired()) {
                    ...
                    for (String node : ClusterManager.getInstance().getPorts()) {
                        if (!node.equals(System.getenv("Node_Port"))) {
                            ...
                            UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl("http://host.docker.internal:" + node + "/document")
                                    .queryParam("databaseName", databaseName)
                                    .queryParam("collectionName", collectionName)
                                    .queryParam("fromAffinityNode", true)
                                    .queryParam("id", DatabaseManager.getInstance().getDatabases().get(databaseName).getCollections().get(collectionName).getDocuments().get(lastId))
